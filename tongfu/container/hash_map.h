...
class HashMap {
   public:
    using Entry = tongfu::Entry<const K, V>; // tongfu:: is necessary
    class Iterator;

   public:
    HashMap(int capacity = 97);
    int size() const;
    bool empty() const;
...
template <typename K, typename V, typename H>
typename HashMap<K, V, H>::Iterator HashMap<K, V, H>::inserter(
    const Iterator& p, const Entry& e) {
